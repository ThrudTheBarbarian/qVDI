...
void ConnectionMgr::_incomingData(void)
	...
	while (bytesAvailable || bufferNotEmpty)
		...
		switch (cm.type())
			...
			// 36
			// ---------------------------------------------------------------
			case ClientMsg::VQM_ATTRIBUTES:
				VDI::sharedInstance().vqm_attributes(io, cm);
				break;

			// 37
			// ---------------------------------------------------------------
			case ClientMsg::VQF_ATTRIBUTES:
			...
			// 38
			// ---------------------------------------------------------------
			case ClientMsg::VST_ALIGNMENT:
				VDI::sharedInstance().vst_alignment(io, cm);
				break;

			// 59
			// ---------------------------------------------------------------
			case ClientMsg::VS_PIXRGB:
