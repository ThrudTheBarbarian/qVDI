...
|*
\*****************************************************************************/
void VDI::v_pline(int handle, int16_t num, int16_t*pxy)
	{
	...
	if (ws != nullptr)
		...
		{
		WARN("v_pline() cannot find workstation for handle %d", handle);
		}
	}
...
void VDI::v_pline(Transport *io, ClientMsg &cm)
	...
	if (num == (int16_t)p.size()-1)
		...
		v_pline(fd, num/2, pxy);
		}
	else
		WARN("v_pline() expected %d args, got %d", num, (int)p.size()-1);
	}
